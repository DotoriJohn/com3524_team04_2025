...
class Grid2D(Grid):
    ...
    def __init__(self, ca_config, transition_func):
        ...
        if not (numrows > 2 and numcols > 2):
            raise ValueError("Invalid grid size {g}".format(g=ca_config.grid_dims))
        ...
        self.wrapping_grid = np.empty((numrows + wrapsize * 2, numcols + wrapsize * 2))
        # initial state fill
        self.wrapping_grid.fill(ca_config.states[0])
        self.grid = self.wrapping_grid[wrapsize:-wrapsize, wrapsize:-wrapsize]
    ...
    def _gen_wrap_indicies(self, wrapsize):
        ...
        # start from wrap-top
        wrapindicies = [
            (0, wrap_height, wrap_width, (-wrap_width)),
            # wrap-bottom
        ...
        gridindicies = [
            (-wrap_height, gridrowmax, 0, gridcolmax),
            # grid-top
...
def randomise2d(grid, background_state, proportions):
    ...
    randindicies = np.random.choice(
        grid.size, np.sum(numcells_per_state), replace=False
    )
